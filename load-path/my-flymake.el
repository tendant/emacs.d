...
(if linuxp
    (setq ecj-jar-file (concat (file-name-directory (or load-file-name buffer-file-name)) "lib/org.eclipse.jdt.core_3.7.0.v_B61.jar")))
(if mac-osx-p 
    (setq ecj-jar-file (concat (file-name-directory (or load-file-name buffer-file-name)) "lib/ecj-3.6.jar")))
...
(defun flymake-java-ecj-init ()
  (let* ((temp-file   (flymake-init-create-temp-buffer-copy
         ...
         (local-file  (file-relative-name
                       temp-file
                       (file-name-directory buffer-file-name)))) ; WHY do I need local-file, it doesn't work for some file in a symlinked directory. 
    ...
    (list "java" (list "-jar" ecj-jar-file
                       ...
                       ;; "-d" (flymake-built-class-path) ;; generate .class file and put it into (flymake-built-class-path))
                       ...
                       "-classpath" 
                       (jde-build-classpath jde-global-classpath) temp-file))))
...
(add-hook 'flymake-mode-hook
          '(lambda nil
             ...
             ;; Don't want to compile java file after every single
             ;; change, only compile after saving file.
...
(defun flymake-javac-current-class ()
  "Compile java source file to classpath"
  (interactive)
  (let* ((source-file-name buffer-file-name)
         (cmd-and-args (funcall 'java-simple-compile-command-and-args))
         (cmd  (nth 0 cmd-and-args))
         (args (nth 1 cmd-and-args)))
    (progn
      (apply 'start-process "flymake-compile-proc" nil cmd args)))
  )

(defun java-simple-compile-command-and-args ()
  (list "java" (list "-jar" ecj-jar-file
                     "-Xemacs" 
                     "-g" ;; all debug info
                     ;; "-d" "none" ;; generate no .class files
                     ...
                     ;;"-sourcepath" (car jde-sourcepath)
                     "-classpath" 
                     (jde-build-classpath jde-global-classpath) buffer-file-name)))
