...
LSP :: Error from the Language Server: Unhandled method textDocument/documentColor (Method not Found)

> Just executed M-x lsp in your current buffer.
...
eshell-aliases-file is a variable defined in ‘em-alias.el’.

Its value is "~/.emacs.d/eshell/alias"
...
(use-package puni
  ...
  (puni-global-mode)
  (add-hook 'eval-expression-minibuffer-setup-hook #'puni-disable-puni-mode) ; Disable puni in minibuffer to avoid conflict with ido C-d keybinding
